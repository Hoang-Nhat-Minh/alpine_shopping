...
<div class="page-content">
  ...
  @php
  // Data for pie chart
  $topProducts = App\OrdersDetail::select('product_name', \DB::raw('SUM(number) as total_number'))
  ->groupBy('product_name')
  ->orderBy('total_number', 'desc')
  ->limit(5)
  ->get();

  $topFiveName = $topProducts->pluck('product_name')->toArray();
  $topFiveNumber = $topProducts->pluck('total_number')->toArray();

  // Data for line chart
  $today = Carbon\Carbon::today();
  $weekAgo = $today->copy()->subDays(6);

  $orders = App\Order::whereBetween('created_at', [$weekAgo, $today->endOfDay()])->get();
  $dailyTotals = [];

  $dates = [];
  $moneyTotals = [];

  for ($date = $weekAgo->copy(); $date->lte($today); $date->addDay()) {
  $dailyTotal = $orders->filter(function ($order) use ($date) {
  return $order->created_at->isSameDay($date);
  })->sum('totalPrice');

  $dates[] = $date->format('d-m');
  $moneyTotals[] = $dailyTotal;
  }


  //TodayOrdersData
  $todayOrdersCount = App\Order::whereDate('created_at', Carbon\Carbon::today())->count();

  //MonthlyData
  $monthlyTotal = App\Order::whereMonth('created_at', Carbon\Carbon::now()->month)
  ...
  $online = \App\AccessTime::where('last_seen', '>=', now()->subMinutes(2))->get()->groupBy('access_user_id')->count();
  ...
  <div class="row" style="padding-top:30px;padding-left:30px;padding-right:30px">
    ...
    <div class="col-md-4">
      <div class="box" id="box1">
        <h4><i class="icon voyager-bag" style="font-size: large; margin-right:5px;"></i> Order hôm nay</h4>
        <hr style="margin: 0">
        <div class="row">
          <div class="col-sm-12 text-center" style="margin-top:5px;margin-bottom:5px">
            <h4 id="todayOrdersCount" style="font-size: xx-large;font-weight: 550;"></h4>
          </div>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="box" id="box2">
        <h4><i class="icon voyager-dollar" style="font-size: large; margin-right:5px;"></i> Thu nhập {{ \Carbon\Carbon::now()->locale('vi')->translatedFormat('F') }}</h4>
        <hr style="margin: 0">
        <div class="row">
          <div class="col-sm-12 text-center" style="margin-top:5px;margin-bottom:5px">
            <h4 id="monthlyTotal" style="font-size: xx-large;font-weight: 550;"></h4>
          </div>
        </div>
      </div>
    </div>
    <div class="col-md-4">
      <div class="box" id="box3">
        <h4><i class="icon voyager-person" style="font-size: large; margin-right:5px;"></i> Số lượt truy cập hôm nay</h4>
        <hr style="margin: 0">
        <div class="row">
          <div class="col-sm-12 text-center" style="margin-top:5px;margin-bottom:5px">
            <h4 style="font-size: xx-large;font-weight: 550;">0</h4>
          </div>
        </div>
      </div>
    </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
      function animateValue(obj, start, end, duration) {
        ...
        const step = (timestamp) => {
          ...
          obj.innerHTML = formatVietnameseNumber(Math.floor(progress * (end - start) + start));
          if (progress < 1) {
      ...
      animateValue(todayOrdersCount, 0, `{{ $todayOrdersCount }}`, 1500);
      animateValue(monthlyTotal, 0, `{{ $monthlyTotal }}`, 1500);
  ...
  <div class="row" style="padding:30px">
    <div class="col-sm-12 col-md-8">
      <div class="panel panel-bordered" style="padding-bottom:5px;">
        <h3 class="text-center">Thu nhập theo ngày năm {{ Carbon\Carbon::now()->year }}</h3>
        <canvas id="line"></canvas>
      </div>
    </div>
    <div class="col-sm-12 col-md-4">
      <div class="panel panel-bordered" style="padding-bottom:5px;">
        <h3 class="text-center">Top 5 sản phẩm</h3>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const topFiveName = @json($topFiveName);
      const topFiveNumber = @json($topFiveNumber);
      const dates = @json($dates);
      const moneyTotals = @json($moneyTotals);
